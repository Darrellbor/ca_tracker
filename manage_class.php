...
<h1>
    ...
    <?php
		$select_class=mysql_query("SELECT * FROM `class` WHERE (status='$status')");
    ...
    <form method="post" id="myform">
      ...
      <script language="javascript">
			...
			{
				document.getElementById("myform").action="manage_class.php?status="+document.getElementById("status").value;
		...
		{
			?>
   				<p class="error" align="center">No class record found! <?php echo mysql_error(); ?></p>
            <?php
			die();
		}
 ...
 <br />
 	...
 	<table align="center" cellpadding="10px" cellspacing="1px">
        ...
        <?php
			$bg_color="";
			while($row_selected_class=mysql_fetch_assoc($select_class))
			{
				$bg_color=($bg_color=="#efefef") ? "#cdcdcd" : "#efefef";
				?>
                	<tr bgcolor="<?php echo $bg_color; ?>">
                    	<td><?php echo $row_selected_class['registration_id']; ?><br />
                        <span class="small"><a href="edit_class.php?registration_id=<?php echo $row_selected_class['registration_id']; ?>">Edit</a></span>
                        </td>
                        <td><?php echo $row_selected_class['class']; ?></td>
                        <td><?php echo $row_selected_class['arm']; ?></td>
                        <td><?php echo $row_selected_class['status']; ?></td>
                    </tr>
                <?php
			}
		?>
