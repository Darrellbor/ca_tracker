...
<body>
<?php
	$select_sql=mysql_query("select * from `saved_events` where (registration_id='$registration_id' and event_code='$event_code')");
	if($select_sql==FALSE)
	{
	...
	$del_saved_event=mysql_query("delete from `saved_events` where (registration_id='$registration_id' and event_code='$event_code')");
