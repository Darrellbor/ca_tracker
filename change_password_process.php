<?php
	...
	//check if the information inputed is legit
		$get_user=mysql_query("select * from `students` where (email='".$_SESSION['current_user_email']."' and password='$current_pass')");
		if($get_user==FALSE)
		{
		...
		$insert_password=mysql_query("update `students` set password='$new_pass' where email='".$_SESSION['current_user_email']."'");
