<?php
	...
	$main_menu="Events";
	$sub_menu="Deleted/Passed Events";
?>
...
<?php echo $_SESSION['current_term']; ?>,<?php echo $_SESSION['current_session']; ?> <br /> 
    ...
    <!-- InstanceBeginEditable name="mycontents" -->
        ...
        <?php
			$select_dp_events=mysql_query("select * from `deleted/passed_events` where registration_id='".$_SESSION['current_user_reg_no']."'");
			if($select_dp_events==FALSE)
			{
				?>
					<p class="error">Error encountered selecting deleted/passed events</p>
                <?php
			}
		?>
        ...
        <form method="post" id="myform">
        	...
        	<?php
				$total_selected_dp_events=mysql_num_rows($select_dp_events);
				if($total_selected_dp_events<=0)
				{
					?>
                    	<p class="error">Sorry you have no deleted or passed events. <?php echo mysql_error(); ?></p>
                    <?php
					die();
				}
			?>
            ...
            <table align="center" cellpadding="10px" cellspacing="1px" id="data_table">
				...
				function add_event_click(reg_id,event_code)
				{
					...
					if(r)
					{
						document.getElementById("myform").action="add_saved_event1.php?reg_id="+reg_id+"&event_code="+event_code;
                ...
                <?php
					$bg_color="";
					while($row_selected_dp_events=mysql_fetch_assoc($select_dp_events))
					{
						$bg_color=($bg_color=="#efefef") ? "#cdcdcd" : "#efefef";
						?>
                        	<tr style="color:#007DFB" bgcolor="<?php echo $bg_color; ?>">
                            	<td><?php echo $row_selected_dp_events['title']; ?> <br />
                                	<span class="small"><a href="Javascript:void(0)" onclick="add_event_click('<?php echo $_SESSION['current_user_reg_no']; ?>','<?php echo $row_selected_dp_events['event_code']; ?>')">Re-instate Event</a></span>
                                </td>
                                <td><?php echo $row_selected_dp_events['description']; ?></td>
                                <td><?php echo $row_selected_dp_events['supposed_date_of_event']; ?></td>
                            </tr>
