...
<?php echo $_SESSION['current_term']; ?>,<?php echo $_SESSION['current_session']; ?> <br /> 
    ...
    <!-- InstanceBeginEditable name="mycontents" -->
    	<p align="left" style="font-size:14px; color:#007DFB;"><a href="home.php">Home</a>>Manage C.A>Test</p><br /> <br />
         <?php
		 	$select_test=mysql_query("select * from `test` where registration_id='$reg_id' and week='$week'");
			if($select_test==FALSE)
			{
				?>
  				 	<p class="error">Error encountered selecting test scores!</p>
                <?php
			}
			...
			{
				?>
                	<p class="error">Error encountered selecting test scores! <?php echo mysql_error(); ?></p>
                <?php
				exit();
			}
         ...
         <form action="test_save_score.php" method="post" id="myform">
         	<p>
         	  <?php
				$total_selected_test=mysql_num_rows($select_test);
				if($total_selected_test<=0)
				{
					 echo create_test($week,$list_of_subjects,$reg_id);
				}
					...
					?>
                    	<table align="center" cellpadding="10px" cellspacing="1px" id="data_table" style="color:#007DFB">
                            ...
                            <?php
								$bg_color="";
								$sub_count=0;
								while($row_selected_test=mysql_fetch_assoc($select_test))
								{
									$sub_count++;
									...
									?>
                                    	<tr bgcolor="<?php echo $bg_color; ?>">
                                       	  <td>
												<?php echo $row_selected_test['subject']; ?>
                                              <input name="subject<?php echo $sub_count; ?>" type="hidden" id="subject<?php echo $sub_count; ?>" value="<?php echo $row_selected_test['subject']; ?>" />
                                              
                                            </td>
                                            <td>
                                            	<?php
													if($row_selected_test['score']<=0)
													{
														for($count_score=0;$count_score<1;$count_score++)
														{
															?>
                                                            	<input type="hidden" value="<?php echo $row_selected_test['test_id']; ?>" id="score_id<?php echo $sub_count; ?>" name="score_id<?php echo $sub_count; ?>" />
                                                            
														<input name="score<?php echo $sub_count; ?>" type='text' id="score<?php echo $sub_count; ?>" placeholder='0.0' style='width:50px;' autocomplete='off' value="<?php echo $row_selected_test['score']; ?>" />
														<?php
														}
													}
