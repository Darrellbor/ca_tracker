<?php
	...
	$main_menu="Assignment";
	$sub_menu="View Assignment Scores";
...
<?php echo $_SESSION['current_term']; ?>,<?php echo $_SESSION['current_session']; ?> <br /> 
    ...
    <!-- InstanceBeginEditable name="mycontents" -->
    	 <p align="left" style="font-size:14px; color:#007DFB;"><a href="home.php">Home</a>>Manage C.A>Assignment</p><br /> <br />
         <?php
		 	$select_assignment=mysql_query("select * from `assignment` where registration_id='$reg_id' and week='$week'");
			if($select_assignment==FALSE)
			{
				?>
  				 	<p class="error">Error encountered selecting assignment scores!</p>
                <?php
			}
			...
			{
				?>
                	<p class="error">Error encountered selecting assignment scores! <?php echo mysql_error(); ?></p>
                <?php
				exit();
			}
         ...
         <form action="assignment_save_score.php" method="post" id="myform">
         	<p>
         	  <?php
				$total_selected_assignment=mysql_num_rows($select_assignment);
				if($total_selected_assignment<=0)
				{
					 echo create_assignment($week,$list_of_subjects,$reg_id);
				}
					...
					?>
                    	<table align="center" cellpadding="10px" cellspacing="1px" id="data_table" style="color:#007DFB">
                            ...
                            <?php
								$bg_color="";
								$sub_count=0;
								while($row_selected_assignment=mysql_fetch_assoc($select_assignment))
								{
									$sub_count++;
									...
									?>
                                    	<tr bgcolor="<?php echo $bg_color; ?>">
                                       	  <td>
												<?php echo $row_selected_assignment['subject']; ?>
                                              <input name="subject<?php echo $sub_count; ?>" type="hidden" id="subject<?php echo $sub_count; ?>" value="<?php echo $row_selected_assignment['subject']; ?>" />
                                              
                                            </td>
                                            <td>
                                            	<?php
													if($row_selected_assignment['score']<=0)
													{
														for($count_score=0;$count_score<1;$count_score++)
														{
															?>
                                                            	<input type="hidden" value="<?php echo $row_selected_assignment['assignment_id']; ?>" id="score_id<?php echo $sub_count; ?>" name="score_id<?php echo $sub_count; ?>" />
                                                            
														<input name="score<?php echo $sub_count; ?>" type='text' id="score<?php echo $sub_count; ?>" placeholder='0.0' style='width:50px;' autocomplete='off' value="<?php echo  $row_selected_assignment['score']; ?>" />
														<?php
														}
													}
													...
													{
														echo  $row_selected_assignment['score'];
													}
												?>
                                            </td>
                                        </tr>
